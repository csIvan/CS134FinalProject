...
void ImpulseRadialForce::updateForce(Particle * particle) {
    ...
    particle->position += ofVec3f(ofRandom(-.2,.2), 0, ofRandom(-.2,.2));
	ofVec3f dir = ofVec3f(0, -3, 0);
...
void Thruster::updateForce(Particle * particle) {
    particle->forces.x += mover.x;
    particle->forces.y += mover.y;
    particle->forces.z += mover.z;
}
